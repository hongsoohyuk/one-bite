...
1. main 브랜치 푸시 또는 수동 트리거
2. Gradle 빌드 + 테스트
3. Docker 이미지 빌드 → GHCR(`ghcr.io/<owner>/onebite-server`) 푸시
4. SSH로 EC2 접속 → `docker compose pull && up -d`
5. `/actuator/health` 재시도 헬스체크

### 필요한 GitHub Secrets
- `EC2_HOST` — Elastic IP
- `EC2_SSH_KEY` — terraform output으로 추출한 private key 전체
...
# 서버 내부에서
cd /opt/onebite
./infra/scripts/init-ssl.sh <your-domain>
```

### 일상 배포
GitHub Actions가 main 브랜치 푸시 시 자동으로 배포합니다. 수동 롤아웃은 Actions 탭에서 `deploy` 워크플로우를 재실행.
